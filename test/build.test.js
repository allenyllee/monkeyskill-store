...
test("Store publishes an immutable hashed Runner Bootstrap package", async () => {
  ...
  const hostProfile = JSON.parse(await readFile(new URL("../dist/skills/monkeyskill-runner-bootstrap/1.0.5/protocol/host-dsl-profile.json", import.meta.url), "utf8"));
  ...
  assert.match(hostProfile.testSpec.behaviorTest.fixtureNode.parent, /null/);
  assert.deepEqual(hostProfile.fieldSemantics.copyShortcutOperation, ["copy", "cut"]);
  assert.equal(hostProfile.fieldSemantics.selectionCollapsed.step, "optional");
  assert.ok(hostProfile.fieldSemantics.hitTest.point.includes("center"));
  assert.match(hostProfile.fieldSemantics.nodeReferences, /node ids/);
  assert.match(hostProfile.fieldSemantics.styleProperties, /camelCase/);
  assert.ok(hostProfile.fieldSemantics.operators.includes("neq"));
