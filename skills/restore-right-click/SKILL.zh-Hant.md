...
- 空 overlay 的判定必須在真正套用修復的當下重新確認。應用程式渲染過程中曾短暫為空、但稍後已含 link、button、表單 control、`details`／`summary` 或其他內容的容器，是正常互動 UI；不得僅因第一次觀察時為空，就對它或其祖先設定 `pointer-events:none`。
...
- [criterion:text-selection] 即使存在 `user-select:none`、`unselectable=on`、primary `mousedown` 或 `selectstart` cancellation，文字仍可選取並保持選取。Standard 的 `mousedown` 與 `selectstart` 必須分開測試；至少一個 `selectstart` 測試必須使用 `installTiming: "before-fixture"`，讓文字目標與 blocker 在 Skill 的 `document_start` 安裝後才一起出現。Absolute release-clear 測試還必須在 `drag-select-text` step 斷言 `selection-write-count lte 5`，避免延後恢復對已存在的 Range 重複寫入並鎖死頁面。
...
- [criterion:pointer-overlays] 修復 image、video、canvas、input、textarea、editable control 的空 overlay 與 `pointer-events:none`，包括動態與 offscreen 情境。至少一個 overlay 測試必須由 fixture author rule 或 class 定位，而非把 position 寫在 inline `style` attribute。Skill 於 `document_start` 執行，不能因為稍後一次加入的頁面 root 已含超過少量後代就整棵略過。另以 `installTiming: "before-fixture"` 測試：候選先安裝，再用單一 mutation 掛入一個含超過 24 個直接後代的 root，其中包含 image、canvas 或 input 與由 author rule 定位的空 overlay，並要求底層目標通過 `hit-test`。固定回歸另建立位於 background target 上、起初為空的應用容器，並在延遲 overlay 處理完成前加入互動 control；該 control 必須保持為 hit-test 結果。實作在改變 pointer 行為前必須重驗目前是否仍為空及是否已有互動後代，並丟棄過期的 overlay 分類。
