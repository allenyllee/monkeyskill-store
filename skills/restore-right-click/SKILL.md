...
- Treat emptiness as a property that must be revalidated at the moment a queued overlay repair is applied. A container that was briefly empty while an application rendered, but now contains a link, button, form control, `details`/`summary`, or any other content, is ordinary application UI: never set `pointer-events:none` on it or on one of its ancestors merely because it was empty when first observed.
...
- For overlays, use geometry-based overlap detection or reliably rescan when an offscreen target
  ...
  time is insufficient. Detect positioning from computed author styles as well as inline styles:
  real pages commonly position an empty overlay through a CSS class, so matching only a
  `style` attribute must not pass validation.
  The Skill runs at `document_start`, before an ordinary page body has necessarily been parsed.
  Do not discard a newly added root merely because it already contains more than a small number
  of descendants. Traverse or index such a subtree incrementally within the same bounded work
  budget. A trusted `before-fixture` test installs the candidate first, then publishes one detached
  page root containing more than 24 direct descendants, including a pointer target and an
  author-styled empty overlay, as a single mutation. The underlying target must become the hit-test
  result without a reload or a later attribute change.
...
- Keep scroll handling bounded independently of total document size. Do not query all generic
  ...
  over 10 scroll frames it must complete within 400 ms. Repeated geometry and hit-test reads in the
  same frame can each force layout; do not call `elementsFromPoint()` or equivalent once per queued
  overlay. Use at most one shared layout/hit-test checkpoint per bounded frame or an equivalent
  repair whose forced-layout count is independent of the number of queued overlays. In the real
  Demo, first record the Skill-Off browser/layout baseline. Standard and Absolute must each add no
  more than 500 ms and remain no slower than 1.5 times that baseline, without delaying native wheel
  scrolling. A single live `DocumentFragment` may add hundreds of sibling roots in one mutation
  delivery. Do not run `querySelectorAll()` separately on every added generic root. Filter roots
  cheaply first, combine candidate discovery across the batch, or process a bounded number per
  task so setup cost does not grow as one expensive subtree query per sibling.
...
- [criterion:text-selection] Selected text remains selected despite `user-select: none`, `unselectable=on`, primary `mousedown` cancellation, or `selectstart` cancellation. Standard-mode tests must independently model both the `mousedown` and `selectstart` blocker families. At least one Standard `selectstart` test must use `installTiming: "before-fixture"`, so the selectable target and blocker arrive after the Skill's `document_start` installation. An Absolute release-clear test must also assert `selection-write-count` at `lte 5` for its `drag-select-text` step so deferred recovery cannot redundantly rewrite an already-live Range and lock the page.
...
- [criterion:pointer-overlays] Empty blocking overlays and `pointer-events: none` targets are repaired for images, video, canvas, input, textarea, and editable controls. Tests must independently include canvas as well as media/editable targets. Include a target that exists when the restoration script starts and an empty covering overlay appended afterward; the underlying target must become the hit-test result. At least one overlay must receive its positioning from a fixture author rule or class rather than an inline `style` attribute. Also cover an initially offscreen target and overlay so implementations cannot rely only on `elementFromPoint()` at insertion time; repair must be geometry-based or reliably rerun when the target enters the viewport. A separate `installTiming: "before-fixture"` test must publish one fixture root with more than 24 direct descendants in a single mutation; among those descendants place an image, canvas, or input beneath an author-styled empty overlay and require the underlying target to pass `hit-test`. A fixed regression must also start with an empty application container over a background target, append an interactive control before deferred overlay processing completes, and require that control to remain the hit-test result. Implementations must revalidate current emptiness and interactive descendants immediately before changing pointer behavior; stale overlay classifications must be discarded.
...
- [criterion:dynamic-performance] Standard and Absolute remain responsive at startup, during sustained dynamic DOM changes, after queued observer repairs, and during native scrolling on large single-page applications. A trusted `startup-stress` workflow with 1200 control/overlay pairs already present before candidate installation, a `mutation-burst` of 200 ID-bearing rows in 20 batches, and a separate realistically laid-out `scroll-stress` workflow that inserts 1200 control/overlay sibling roots into a live container in one `DocumentFragment` and then runs 10 scroll frames each include document-wide DOM-quiet and forced-layout checkpoints and complete within 400 ms. The implementation does not synchronously walk every element of an already-large page, recursively drain `document.documentElement` at startup, call `getComputedStyle()` on every newly added generic descendant, run a separate `querySelectorAll()` subtree scan for every sibling root in a large mutation delivery, process hundreds of style/layout reads in one flush, call `elementsFromPoint()` or an equivalent forced-layout hit test once per queued overlay, enter a self-triggering observer loop, enumerate document IDs into a growing selection stylesheet, rebuild cumulative ID selector lists across multiple flushes, iterate every accumulated overlay during mutation flushes, rebuild a global target index on every scroll, perform document-wide geometry cross-products, delay wheel scrolling, or leave ordinary page interaction unresponsive. Test all three workflows independently in both modes. For the real Demo, first record the Skill-Off baseline; each enabled mode must add no more than 500 ms and remain no slower than 1.5 times that baseline, without a persistent busy cursor or multi-second main-thread stalls.
