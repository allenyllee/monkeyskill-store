const translations = {
  "zh-Hant": {
    ...
    waiting: "等待操作…",
    hints: { 5: "通過條件：不出現 alert，且原生右鍵選單正常開啟。", 6: "右鍵目標應恢復成圖片。", 8: "右鍵目標應恢復成圖片。", 12: "拖曳選取並觀察反白底色；Dark Reader 等色彩擴充可能干擾此項。", 13: "Absolute 應讓 input 重新成為事件目標。", 15: "右鍵目標應恢復成 canvas。", 16: "右鍵應開啟，但「另存新檔」會儲存 HTML，因為它不是 img。", 17: "計時包含排隊中的 observer 工作直到 DOM-quiet checkpoint，並應在 1000 ms 內完成。", 18: "先以 Skill-Off 量基線；啟用後額外耗時不得超過 500 ms、總時間不得超過 1.5 倍，且不得延遲原生滾輪。" },
    ...
    alert: "失敗：頁面的右鍵 alert handler 仍然執行。"
  },
  en: {
    ...
    alert: "Failed: the page's context-menu alert handler still ran."
  }
};
