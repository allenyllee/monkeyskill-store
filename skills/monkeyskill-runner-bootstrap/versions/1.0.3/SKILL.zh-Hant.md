...
[criterion:host-integration] 設定 MonkeySkill 已認證的本機 Agent API，透過有界 stdin/stdout 協定呼叫 active Runner。生成交付必須包含或取代必要的 Host adapter，以接受 `protocol/host-dsl-profile.json` 描述的 MonkeySkill 正規化 Developer Conformance request；只測 Runner 私自近似的 request 不算通過。必須證明真正安裝後的 Store→Host→生成 Runner 路徑（不只是直接 Runner harness）可接受 `build.modes`、fixture nodes／rules／blockers、安裝時機，以及 profile 必要相容矩陣中的每種 action／assertion。回傳 evidence 的 artifact hash 必須與安裝版本相符，證明實際呼叫的是生成 artifact。Host adapter 與 Runner 必須安全編譯並執行其宣告支援的每一種 TestSpec action 與 assertion；meta-conformance 必須包含正向的動態 `append-node` 往返案例，證明新加入的控制項仍可命中與取得焦點。若 action、assertion、capability、schema、adapter compilation 或 provider operation 不受支援，這是 Runner 基礎設施失敗，必須在耗用應用 Builder attempt 前停止，絕不可投影成應用 criterion 失敗。只有已實際執行且失敗的受支援 assertion，才可依原測試的 criterion、mode、assertion type 與固定 category 轉譯成 MonkeySkill 受限詞彙；不得傳遞 provider 訊息、fixture、actual／expected 值或修復指令。Meta-conformance 必須證明刻意失敗的受支援 assertion 會產生非空且符合 schema 的受限 Builder 診斷。在 required generated-Runner mode 下不得偷偷 fallback 到預寫的 real-browser Runner。
